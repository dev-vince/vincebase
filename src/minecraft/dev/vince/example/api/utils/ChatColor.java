...
public enum ChatColor
{
    ...
    public final ChatColor getByChar(final char code) {
        return ChatColor.BY_CHAR.get(code);
    }

    public final ChatColor getByChar(final String code) {
        Validate.notNull(code, "Code cannot be null");
        Validate.isTrue(code.length() > 0, "Code must have at least one char");
        return ChatColor.BY_CHAR.get(code.charAt(0));
    }

    public final String stripColor(final String input) {
        if (input == null) {
            return null;
        }
    ...
    public final String translateAlternateColorCodes(final char altColorChar, final String textToTranslate) {
